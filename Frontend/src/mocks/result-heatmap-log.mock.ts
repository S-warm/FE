...
function generateSessions(seed: number, sessionCount: number) {
  ...
  for (let index = 0; index < sessionCount; index += 1) {
    ...
    const hasBlock = random() < 0.18
    const blockTime = hasBlock ? Math.floor(timelineMaxMs * (0.35 + random() * 0.45)) : null

    const hotspotA = { x: 0.52, y: 0.42 }
    const hotspotB = { x: 0.28, y: 0.58 }
    const hotspotC = { x: 0.74, y: 0.28 }
    ...
    for (let e = 0; e < eventCount; e += 1) {
      ...
      const dwellMs = 120 + Math.floor(random() * 2200)
      const retries = Math.floor(random() * (bias < 0.6 ? 4 : 2))
