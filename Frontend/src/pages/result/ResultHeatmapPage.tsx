...
function HeatmapCanvas({
  ...
  return (
    ...
    >
      {/* 스크린샷 컨테이너 — 필터/데이터 상태와 무관하게 항상 렌더링 */}
      <div className="inline-block w-full">
        <div className="relative w-full" style={{ display: "inline-block" }}>
          {activeScreenshotUrl && failedScreenshotUrl !== activeScreenshotUrl ? (
            ...
            /* URL 없거나 로드 실패 시 — 레이아웃 유지용 플레이스홀더 */
            <div className="w-full bg-surface-subtle" style={{ minHeight: "400px" }}>
              <div className="flex h-full min-h-[400px] items-center justify-center">
                <p className="text-caption-12-regular text-text-muted">스크린샷이 없습니다</p>
              </div>
            </div>
          )}

          {/* 히트맵 캔버스 오버레이 — 이미지 메트릭 있을 때만 */}
          {imageMetrics ? (
            <canvas
              ref={canvasRef}
          ...
          {/* 핀포인트 마커 오버레이 — points 있을 때만 */}
          {imageMetrics && isPinpointMode && page.points.length > 0 ? (
            <div
...
function ResultHeatmapPage() {
  ...
  // 마지막으로 로드된 페이지 목록 (히트맵 데이터용)
  const lastKnownPagesRef = useRef<ResultHeatmapPageViewModel[]>([])
  // 사이드 패널 전용 전체 페이지 목록 — 줄어들지 않음, 필터와 무관
  const basePagesRef = useRef<ResultHeatmapPageViewModel[]>([])

  if (pages.length > 0) {
    lastKnownPagesRef.current = pages
    // 더 많은 페이지가 있을 때만 업데이트 (페이지가 줄어드는 건 반영 안 함)
    if (pages.length >= basePagesRef.current.length) {
      basePagesRef.current = pages
    }
  }
  ...
  const selectedPage = useMemo<ResultHeatmapPageViewModel | null>(() => {
    // 스크린샷/레이아웃 기준은 sidePanelPages (항상 전체)
    const basePageLayout =
      sidePanelPages.find((page) => page.pageId === selectedPageId) ?? sidePanelPages[0] ?? null
    if (!basePageLayout) return null

    // 히트맵 포인트는 필터 결과(displayPages)에서 조회
    const filteredPage = displayPages.find((page) => page.pageId === selectedPageId)

    // 필터 전부 해제 또는 해당 페이지에 필터 결과 없으면 → 포인트만 빈 배열
    if (selectedAgeBands.length === 0 || !filteredPage) {
      return { ...basePageLayout, points: [] }
    }

    return filteredPage
  }, [sidePanelPages, displayPages, selectedPageId, selectedAgeBands])
  ...
  // 캐시된 데이터가 없을 때만 스켈레톤 표시 (필터 변경 중엔 기존 화면 유지)
  if (isLoading && lastKnownPagesRef.current.length === 0) {
